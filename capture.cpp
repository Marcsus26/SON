...
struct mydsp : public dsp {
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = fConst0 * std::pow(1e+01f, 0.05f * static_cast<float>(fHslider0));
	...
	#define FAUST_FILE_NAME "capture.dsp"
	...
	FAUST_ADDBUTTON("Audio Capture/Capture", fButton0);
	FAUST_ADDHORIZONTALSLIDER("Audio Capture/level (db)", fHslider0, 0.0f, -96.0f, 4.0f, 0.1f);

	#define FAUST_LIST_ACTIVES(p) \
		p(BUTTON, Capture, "Audio Capture/Capture", fButton0, 0.0f, 0.0f, 1.0f, 1.0f) \
		p(HORIZONTALSLIDER, level_(db), "Audio Capture/level (db)", fHslider0, 0.0f, -96.0f, 4.0f, 0.1f) \
...
capture::capture() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void capture::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void capture::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float capture::getParamValue(const std::string& path)
{
